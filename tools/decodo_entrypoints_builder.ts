...
(async() => {
    const sourceRaw = await fs.readFile('tools/decodo_entrypoints.json');
    // eslint-disable-next-line @typescript-eslint/naming-convention
    ...
    const entryPointsFiltered = source.entry_points.filter((ep) => {
        return ![
            'gate.decodo.com', 'state.decodo.com', 'city.decodo.com',
        ].includes(ep.domain);
    });
